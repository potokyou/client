...
import org.potok.vpn.protocol.getStatistics
import org.potok.vpn.protocol.getStatus
import org.potok.vpn.qt.QtAndroidController
...
private const val TAG = "PotokActivity"
...
class PotokActivity : QtActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d(TAG, "Create Potok activity: $intent")
    ...
    override fun onStart() {
        super.onStart()
        Log.d(TAG, "Start Potok activity")
        mainScope.launch {
            qtInitialized.await()
            vpnProto?.let { proto ->
                if (PotokVpnService.isRunning(applicationContext, proto.processName)) {
                    doBindService()
                }
            }
        }
    ...
    override fun onDestroy() {
        Log.d(TAG, "Destroy Potok activity")
        ...
        VpnService.prepare(applicationContext)?.let { intent ->
            startActivityForResult(intent, CHECK_VPN_PERMISSION_ACTION_CODE, ActivityResultHandler(
                onSuccess = {
                    ...
                    Toast.makeText(this@PotokActivity, resources.getText(R.string.vpnGranted), Toast.LENGTH_LONG).show()
                    onPermissionGranted()
                },
                onFail = {
